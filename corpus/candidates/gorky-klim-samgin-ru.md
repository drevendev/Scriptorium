...
This is still **candidate-specific inferred reconstruction** rather than historical deployment proof. The exact MediaWiki-core and Poem versions used by Russian Wikisource for the historical render remain unproven, so `historical_render_equivalence_proven=false`. The resolved Part 2 identity is a wikitext/source-graph identity, **not yet a literary-body identity**: fail-closed literary extraction for Parts 1–4 and deterministic `1 -> 2 -> 3 -> 4` composition remain separate prerequisites before any raw or `scriptorium-text-v1` composite digest can be claimed. FantLab's analyzer-input identity also remains unknown.

## Four-parent extraction prerequisite surface

Before implementing literary-body extraction, Scriptorium now replay-freezes the exact **source-free structural surface** of all four parent revisions in `source-edition-traces/gorky-klim-samgin-ru.body-surface.json`. Across the parents the observed template inventory is `Жизнь Клима Самгина` ×4, direct `poemx1` ×7, `примечания` ×2, `Ко` ×1, plus the single Part 2 target-only `#lst`. The observed HTML-tag inventory is `center` ×6, `ref` ×10 and `nowiki` ×50; Part 1 also has five level-three headings. All four parents have one trailing category link and no table starts/ends.

The direct parent `poemx1` calls are now independently bound source-free: **five in Part 1 and two in Part 2**. Every one has exactly two anonymous arguments, zero named arguments, explicitly empty parameter `1`, and a defined non-empty parameter `2`; the artifact persists offsets, counts and SHA-256 identities rather than prose.

This exposes an important correctness boundary in the previous Part 2 wording. The two direct Part 2 parent calls occupy offsets `25519..25540` and `25568..26321`, while the target-only `#lst` call is at `581758..581810`. Neither direct call overlaps the substitution span. Therefore replacing the `#lst` target **necessarily preserves both parent-level `poemx1` calls**: the resolved Part 2 identity above is a target-substitution reconstruction, not evidence that the entire parent became template-free. The next extractor must handle those direct parent calls together with the other frozen scaffolding/markup instead of silently assuming them away.

No literary body or composite digest is frozen by this prerequisite. Historical MediaWiki/Poem deployment equivalence and FantLab input identity remain unknown, and M2 does not move.

Accordingly the retained status is:

- `source_identity_status = revision_wikitext_source_graph_target_only_lst_poemx1_resolution_body_surface_frozen_body_unfrozen`
