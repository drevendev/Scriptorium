...
This closes the previously open `#tag:poem` argument/attribute-surface prerequisite more strongly than expected: no candidate-specific attribute branch remains.

## Bounded MediaWiki/Poem render surface

The next replay, `source-edition-traces/gorky-klim-samgin-ru.poem-render-surface.json`, re-opens the six exact parameter-2 values only in memory and checks the additional syntax surfaces that could make `Parser::internalParse` change them. Across all six values the conservative active-syntax inventory is **zero**: no template/parameter opens, HTML angle brackets, table starts, horizontal-rule lines, behavior switches, heading lines, internal links, apostrophe markup, external-link starts, raw protocols, magic-link tokens, language-converter opens, or entity references are present.

The Poem-specific branches are equally narrow: no value reaches indentation, leading-space, horizontal-rule or trim behavior. Because the reached tag has zero attributes, `compact` is absent. Under the inferred upstream Poem anchor `03b3694613e23efa1254d8bbbb98121efaef2cb0`, the only content-changing path for these exact values is therefore the **nine observed newlines becoming nine protected `<br />` breaks**, followed by the standard `div.poem` wrapper. The source-free reconstruction freezes per-fragment post-unstrip SHA-256 identities; together the six reconstructed fragments are **534 characters / 799 UTF-8 bytes**.

This is intentionally **candidate-specific inferred reconstruction**, not a generic MediaWiki implementation and not historical deployment proof. The Poem anchor is not evidence that Russian Wikisource actually deployed that exact extension commit, and the referenced MediaWiki core source defines the parser stages screened here rather than identifying the November 2024 deployed core revision. Consequently `historical_render_equivalence_proven=false` even though the materially reached plain-value parser/Poem branches are now reproducible and fail closed on drift.

## Target-only Part 2 resolution

The next bounded replay, `source-edition-traces/gorky-klim-samgin-ru.part2-resolution.json`, combines only the already-frozen source graph above. It re-fetches the exact parent, target dependency and inferred `poemx1` template anchor, reproduces each of the six full template outputs with lazy branch evaluation, verifies every reached Poem fragment against the previous render-surface digest, and substitutes those outputs into the pinned target. Unreached `#expr` / `#iferror`, ordinary templates, magic words, or any new parser surface fail closed instead of being guessed.

The expanded target dependency is now source-freezeable at **585,113 characters / 1,059,957 UTF-8 bytes**, SHA-256 `a027dfe0d867572f3802050fcfd72b07a43fa6efe539d58035b3af59f49a04d4`. After the six substitutions it contains **zero remaining `poemx1` calls and zero `{{` / `}}` expansion delimiters**. Replacing the exact target-only `#lst` call in parent revision `5198033` with that expanded target yields a resolved Part 2 parent-wikitext identity of **1,166,949 characters / 2,114,262 UTF-8 bytes**, SHA-256 `ab45577f567ce504e24936f73fcdff261780168414e4bccc04bbf8927299d016`.

This is still **candidate-specific inferred reconstruction** rather than historical deployment proof. The exact MediaWiki-core and Poem versions used by Russian Wikisource for the historical render remain unproven, so `historical_render_equivalence_proven=false`. The resolved Part 2 identity is a wikitext/source-graph identity; the later literary-body layer below is intentionally a separate candidate-specific extraction contract. FantLab's analyzer-input identity also remains unknown.

## Four-parent extraction prerequisite surface

Before implementing literary-body extraction, Scriptorium replay-froze the exact **source-free structural surface** of all four parent revisions in `source-edition-traces/gorky-klim-samgin-ru.body-surface.json`. Across the parents the observed template inventory is `Жизнь Клима Самгина` ×4, direct `poemx1` ×7, `примечания` ×2, `Ко` ×1, plus the single Part 2 target-only `#lst`. The observed HTML-tag inventory is `center` ×6, `ref` ×10 and `nowiki` ×50; Part 1 also has five level-three headings. All four parents have one trailing category link and no table starts/ends.

The direct parent `poemx1` calls are independently bound source-free: **five in Part 1 and two in Part 2**. Every one has exactly two anonymous arguments, zero named arguments, explicitly empty parameter `1`, and a defined non-empty parameter `2`; the artifact persists offsets, counts and SHA-256 identities rather than prose.

This exposes an important correctness boundary in the earlier Part 2 reconstruction. The two direct Part 2 parent calls occupy offsets `25519..25540` and `25568..26321`, while the target-only `#lst` call is at `581758..581810`. Neither direct call overlaps the substitution span. Therefore replacing the `#lst` target **necessarily preserves both parent-level `poemx1` calls**; the literary extractor handles those direct calls separately instead of assuming them away.

## Frozen four-part literary-body identity

The fail-closed extractor is now replay-bound to the exact five-revision source graph and freezes a **source-free candidate-specific literary-body reconstruction** in `source-edition-traces/gorky-klim-samgin-ru.literary-body.json`. It revalidates and inserts the six dependency and seven direct-parent plain `poemx1` parameter-2 values, applies the bounded Part 2 partial-transclusion layer, removes only the frozen Wikisource scaffolding/editorial markup, and composes Parts `1 -> 2 -> 3 -> 4` with exactly two LF characters between parts.

| Part | Literary-body characters | UTF-8 bytes | Raw / `scriptorium-text-v1` SHA-256 |
| --- | ---: | ---: | --- |
| 1 | 964,215 | 1,744,650 | `1a41cef4b2383a149077da372d9a44ab48c3b2616064da6c1ab5f01a6062f4c8` |
| 2 | 1,164,870 | 2,111,745 | `75896b6a0a87bc10087ececbeb919077de58c628f646aab8994cf653edc15794` |
| 3 | 680,940 | 1,234,441 | `7dc6030381371cdcff5a27a872ab1fd6c49840655e3ce11a45d647ea0bf208ab` |
| 4 | 1,000,587 | 1,868,088 | `237b2119e04bdf8c02dded3d138fda457d8f423012d2abe9d752a07c038c8948` |

The ordered composite is **3,810,618 characters / 6,958,930 UTF-8 bytes**. Raw and `scriptorium-text-v1` normalized SHA-256 are both **`4f8b61e05cf7d96485d9edc0854d99d994da6ba146fabc3f6a3d509110ce4288`**. The committed artifact contains no source prose, only revision identities, extraction-surface facts, counts and digests.

This identity is **not** a historical-render or FantLab-source claim. Its evidence class remains `candidate_specific_inferred_reconstruction`: exact historical Russian Wikisource MediaWiki-core and Poem deployment revisions are unproven, and FantLab does not disclose the immutable analyzer input for its 2022 result. Therefore this capture makes the candidate diagnostic-ready only; it does not make it M2-admissible and does not justify tuning toward FantLab's displayed character count.

Accordingly the retained status is:

- `source_identity_status = revision_wikitext_source_graph_and_candidate_specific_literary_body_frozen`
- `fantlab_source_edition_match = unknown`
- `diagnostic_ready = true`
- `gate_ready = false`
- `m2_parity_admissible = false`
- main parity-catalog admission remains blocked on independent FantLab analyzer-input/source-edition evidence.

The provenance record is `source-edition-traces/gorky-klim-samgin-ru.json`; the source-graph summary is `source-edition-traces/gorky-klim-samgin-ru.revisions.json`; the dependency revision is `source-edition-traces/gorky-klim-samgin-ru.part2-part2.revision.json`; the target-only transclusion contract is `source-edition-traces/gorky-klim-samgin-ru.part2-lst.json`; the historical `poemx1` anchor/raw-shape evidence is `source-edition-traces/gorky-klim-samgin-ru.poemx1-template.revision.json` plus `source-edition-traces/gorky-klim-samgin-ru.poemx1-template.shape.json`; the post-inclusion graph is `source-edition-traces/gorky-klim-samgin-ru.poemx1-template.transclusion.json`; the literal parameter surface is `source-edition-traces/gorky-klim-samgin-ru.poemx1-template.parameters.json`; concrete invocation/frame bindings are `source-edition-traces/gorky-klim-samgin-ru.poemx1-template.bindings.json`; frozen parser-control reachability is `source-edition-traces/gorky-klim-samgin-ru.poemx1-template.control.json`; the source-free parameter-2 surface is `source-edition-traces/gorky-klim-samgin-ru.poemx1-content.json`; the reached extension-call surface is `source-edition-traces/gorky-klim-samgin-ru.poem-tag-surface.json`; the bounded recursive-parse/Poem reconstruction is `source-edition-traces/gorky-klim-samgin-ru.poem-render-surface.json`; the bounded target-only Part 2 resolution is `source-edition-traces/gorky-klim-samgin-ru.part2-resolution.json`; the four-parent pre-extraction structural surface is `source-edition-traces/gorky-klim-samgin-ru.body-surface.json`; and the canonical source-free literary-body identity is `source-edition-traces/gorky-klim-samgin-ru.literary-body.json`. No source prose is committed. M2 remains **0/5 source-matched works**.
