...
LAST_COMMITTED_RUN_AT: 2026-09-21T00:55:00Z
LAST_RESULT: SCRIP-CORPUS-045 / Issue #177 completed. Independent exact-head review read PR #178 at `b8238144caeb23256eae6a61a49e47b07c687f29` against unchanged base `87929007415ede118cc5700a1e34f2cb3cfc576f`, confirmed 11 commits ahead / 0 behind, 11 changed files, no inline review threads, and all 16 PR-triggered workflows settled `success`. Dedicated Darwin/Rachinsky render-profile run `35546120433` / job `106172226254` checked out the exact reviewed SHA and passed 7 render-profile tests, 5 provenance tests and deterministic profile rebuild. Artifact `10616812708` was independently downloaded and verified as a 1,914-byte source-free ZIP / SHA-256 `12f3be444a4565f7aee5413aef9183487a072bfe9de47ae6b6d27fec05474fbb`, containing only the 12,592-byte render-profile JSON whose canonical embedded SHA-256 recomputes to `2ebcfea51282505c28824f54f6a1795bd7e87e2f958ac59f5c843e68c2066f3d`. Pages run `35546120447` / job `106172226404` checked out the same SHA and passed the full 376-test standard-library suite, canonical site build and deterministic rebuild. Review was recorded as COMMENT `5262352606`, not self-approval; PR #178 was marked Ready and squash-merged as `5e3833e96fd5e28fe19d24c8e20798fa54eadca4`, closing Issue #177 completed.
LAST_VERIFIED_PROGRESS: Master now retains an independently reviewed source-free fail-closed rendering decision profile for the 1864 Rachinsky translation of Darwin's *On the Origin of Species*, cryptographically bound to the reviewed exact-388 markup-surface freeze. The profile classifies all 18 observed tag shapes / 5,164 tokens and all 38 template shapes / 4,583 invocations while leaving 5 math/reference tag shapes / 518 tokens and all template semantics unresolved; `nop` remains explicitly inter-page-sensitive. `rendering_profile_frozen=true` is intentionally distinct from `renderer_semantics_complete=false`, `renderer_implementation_ready=false`, `rendering_equivalence_claimed=false` and `inter_page_composition_frozen=false`. No Page wikitext, template argument values, OCR, rendered prose, scan bytes or literary text are retained. Literary-body counts/digests, >=300k admission, FantLab source match, diagnostic readiness and M2 parity remain closed; M2 remains 0/5.

## Current unit

```text
UNIT_ID:        SCRIP-CORPUS-045
ISSUE:          #177 (closed completed)
STATUS:         COMPLETE
PR:             #178 (squash-merged as 5e3833e96fd5e28fe19d24c8e20798fa54eadca4)
NEXT_ACTION:    Resume normal-flow selection from the queue. A later bounded Darwin/Rachinsky unit may resolve and
                independently freeze provider/template/reference/math semantics explicitly left unresolved by the
                reviewed fail-closed profile; inter-page separator/composition semantics remain a separate subsequent
                decision. Keep literary-body, >=300k admission, FantLab source-match, diagnostic and M2 gates closed
                until their evidence is independently verified.
```
...
- **On the Origin of Species — Rachinsky translation** — SCRIP-CORPUS-045 complete. Master retains 418 exact Page revision identities, reviewed 388-literary/30-apparatus composition contract, exact-388 source-free markup inventory, independently reviewed backing Commons DjVu identity (27,368,263 bytes; SHA-1 `75ef508588194ae74874272ce290f3ec1043ea9b`; SHA-256 `7f3ae1aadd4a844b193783a0c350e23815a76e7bb14a92bae7fa1c077354a2c8`) and the independently reviewed fail-closed render decision profile SHA-256 `2ebcfea51282505c28824f54f6a1795bd7e87e2f958ac59f5c843e68c2066f3d`. The profile covers all 18 tag / 38 template shapes while leaving 5 math/reference tag shapes / 518 tokens and all 38 template shapes / 4,583 invocations unresolved. Renderer implementation/equivalence, inter-page composition, literary-body count/digests and >=300k admission remain unfrozen; `admitted_for_calibration=false`. FantLab work 969964 currently exposes Timiryazev rather than Rachinsky; source match/M2 remain unknown/zero.
...
- SCRIP-CORPUS-045 now exposes the independently reviewed Darwin/Rachinsky exact-388 fail-closed rendering decision profile and synchronized structured provenance, including unresolved 38 template shapes / 4,583 invocations and 5 math/reference tag shapes / 518 tokens, without claiming renderer/body equivalence or corpus admission.
...
14. Darwin/Rachinsky 1864 now has reviewed source-free 388/30 composition, exact-388 markup inventory, backing scan identity and fail-closed decision profile. All 38 template shapes / 4,583 invocations plus math/reference semantics and inter-page composition still require evidence before deterministic body rendering; body count/digests and >=300k admission remain unresolved, and FantLab's current Russian translation surface is a different translation.
