...
LAST_COMMITTED_RUN_AT: 2026-09-17T21:50:00Z
LAST_RESULT: SCRIP-CORPUS-020 / Issue #109 / PR #116 received an independent later-run review of exact head `fcc31b5163d56e6405a646fa88cad95d46e2432a` with no blocking defect, was marked Ready, and was squash-merged as `440765ce082fb6857804dd2bddbeff0b96b7e819`. The merged prerequisite source-freezes the parser-control graph forced by all six already-frozen `poemx1` frames: reachable `#if` x4, `#ifeq` x3 and `#tag:poem` x1; both `#expr` calls, the `#iferror` call, one `#if` and two `#ifeq` occurrences are unreachable. The sole live historical template-expansion boundary is recursive expansion of parameter `2` plus the evidenced `#tag:poem` / poem-extension behavior. Resolved Part 2 bytes, historical render equivalence, literary-body identity and FantLab source equivalence remain explicitly unresolved.
LAST_VERIFIED_PROGRESS: Immediately before merge, PR #116 was mergeable, 9 commits ahead / 0 behind current master `3f2312675aca5d1fe60a716c818b45110d874761`, with 9 changed files and no inline review threads. Exact-head workflows `35274153400` (Klim poemx1 control flow), `35274153394` (invocation bindings), `35274153427` (historical template dependency), `35274153403` (Klim source revisions), `35274153406` (Pages), `35274153398` (frozen diagnostic), and `35274153412` (pinned pylem provider) all completed successfully. The dedicated control job checked out exact head `fcc31b5163d56e6405a646fa88cad95d46e2432a`, ran 217 standard-library tests, regenerated `gorky-klim-samgin-ru.poemx1-template.control.json` from committed prerequisite evidence, and byte-compared it to the committed artifact. Independent review also re-checked exact Russian Wikisource `Шаблон:Poemx1` oldid `5142743`; its source structure independently yields the same reachable/unreachable control graph under the frozen frame. No FantLab source identity was promoted and M2 remains 0/5.
...
ISSUE:          #109
STATUS:         POEMX1_CONTROL_FLOW_MERGED_CONTINUATION_OPEN
PR:             #116 (merged)
MERGED_COMMIT:  440765ce082fb6857804dd2bddbeff0b96b7e819
NEXT_ACTION:    Continue Issue #109 with only the one live historical expansion path: recursive expansion of parameter 2
                as required by #tag:poem plus the evidenced historical poem-extension behavior. Do not manufacture
                #expr/#iferror implementation work for branches proven unreachable under all six frozen calls, and do not
                claim resolved Part 2 bytes until the live tag path replays deterministically. Only then proceed to
                fail-closed four-part extraction/composition.
```
...
- SCRIP-CORPUS-020 first pinned the Klim Part 2 dependency, PR #111 corrected the public explanation to target-only `#lst`, PR #112 exposed the exact source-free `poemx1` as-of anchor, PR #113 exposed the post-inclusion effective dependency boundary, PR #114 exposed the literal parameter surface, merged PR #115 exposed concrete invocation/frame binding, and merged PR #116 now exposes source-free control reachability. The public candidate and trace show that `#expr`/`#iferror` are dead for all six frozen frames and that `#tag:poem` with parameter `2` is the sole live unresolved template operation. Historical render equivalence and Part 2/body identity remain explicit unresolved boundaries.
...
9. Klim Samgin's parent/dependency revisions, target-only `#lst` semantics, inferred historical `poemx1` revision, inclusion controls, post-selection digest, literal parameter-reference/default surface, six concrete invocation bindings and frozen-frame `#if`/`#ifeq` control reachability are now source-frozen. Both `#expr` calls and `#iferror` are unreachable for all six frozen frames; the remaining live historical expansion boundary is recursive expansion of parameter `2` plus `#tag:poem` extension behavior. Resolved Part 2 bytes, candidate-specific extraction/composition and composite raw/normalized body digests remain unfrozen; no historical render or FantLab-input identity may be inferred yet.
