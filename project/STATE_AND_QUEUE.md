...
LAST_COMMITTED_RUN_AT: 2026-09-21T08:58:00Z
LAST_RESULT: SCRIP-CORPUS-048 / Issue #183 authored on Draft PR #184. Official MediaWiki `Help:History` permanent revision `oldid=8524540` establishes that old page revisions render with current template/image versions, while `Transclusion/en` permanent revision `oldid=8551508` describes transclusion as a live link and references the lack of versioned transclusion support (`T31051`). The Darwin/Rachinsky `{{ё}}` evidence boundary is advanced to `scriptorium-darwin-template-yo-documentation-evidence-v2` with canonical SHA-256 `d92bdfcad71aaa8a50f9f35b5b1ada462ed41619400c40576d0603cf154877dc`. The obsolete Page-save-time template-history prerequisite is removed: deterministic replay must instead freeze exact `Шаблон:ё` / `Шаблон:ЕЁ` revisions plus nested dependencies used by the chosen replay environment, or use explicit version-pinned expansion. `{{ё}}` remains unresolved and all downstream gates remain closed. PR #184 is Draft for independent later review; the authoring run does not self-approve or merge.
LAST_VERIFIED_PROGRESS: The project now has a source-free correction to the MediaWiki dependency model underlying old Page revisions: retained Page `oldid`s freeze Page wikitext but do not bind transcluded template revisions. The documented `{{ё}}` class remains conditional `е`/`ё`; however the exact replay-time template dependency graph is not yet frozen, so all 43 backlog shapes remain unresolved and `ё` remains the priority-1 provider-template target with 2,227 invocations. Renderer semantics/implementation/equivalence, inter-page composition, literary-body counts/digests, >=300k admission, FantLab source match, diagnostic readiness and M2 parity remain closed; M2 remains 0/5.
...
PR:             #184 (Draft)
NEXT_ACTION:    Independently re-read the exact final PR #184 head against unchanged master, inspect settled CI and the
                rebuilt source-free evidence artifact, and verify that the MediaWiki oldid/transclusion correction is
                both evidence-backed and fail-closed. If clean, a later judgement run may mark Ready and merge. Do not
                promote {{ё}} until replay-time template/dependency identities (or explicit version-pinned expansion)
                are frozen and validated; keep renderer/body/>=300k/FantLab/diagnostic/M2 gates closed.
```
...
- **On the Origin of Species — Rachinsky translation** — SCRIP-CORPUS-048 is review-pending on PR #184. The branch retains the independently reviewed 418 exact Page revision identities, 388-literary/30-apparatus composition, exact-388 source-free markup inventory, backing Commons DjVu identity, fail-closed render profile SHA-256 `2ebcfea51282505c28824f54f6a1795bd7e87e2f958ac59f5c843e68c2066f3d`, unresolved-semantic backlog SHA-256 `4ec4f84566f5f3f8dccd1945906d1a782f4532547a13cadc8570cb5b682bdcb2`, and proposed source-free `{{ё}}` evidence v2 SHA-256 `d92bdfcad71aaa8a50f9f35b5b1ada462ed41619400c40576d0603cf154877dc`. Wikisource documentation still establishes conditional `е`/`ё` semantics. The new MediaWiki evidence corrects the replay model: Page `oldid` does not pin transcluded template revisions, so Page-save-time template-history lookup is not a valid deterministic replay binding. The exact replay-time `Шаблон:ё` / `Шаблон:ЕЁ` dependency graph is not yet frozen; all 43 shapes remain unresolved and `ё` stays priority 1 at 2,227 invocations. Renderer implementation/equivalence, body count/digests, >=300k admission and FantLab source match remain closed; M2 weight remains zero.
