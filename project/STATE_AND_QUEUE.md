...
LAST_COMMITTED_RUN_AT: 2026-09-17T19:55:00Z
LAST_RESULT: SCRIP-CORPUS-020 / Issue #109 / PR #115 received an independent later-run review of exact head `6141931a13043fa3225971f10ff6c3024a40042a` with no blocking defect, was marked Ready, and was squash-merged as `16cd22d4f3035bb2b3ec83e810cbe41048015486`. The merged prerequisite freezes source-free concrete argument/frame binding for all six `poemx1` invocations in pinned Part 2 dependency oldid `2366546`: each call has exactly two anonymous arguments and zero named arguments, parameter `1` is explicitly defined empty in all six, parameter `2` is defined non-empty in all six, and frozen surface names `3`, `fixed`, `poem`, `small`, and `width` are omitted in all six. Recursive inserted-value expansion, parser functions, `#tag:poem`, historical render equivalence, resolved Part 2 bytes, literary-body identity and FantLab source equivalence remain explicitly unresolved.
LAST_VERIFIED_PROGRESS: Immediately before merge, PR #115 was mergeable, 9 commits ahead / 0 behind current master `ae8fa2944ca60432589def4dc0f5804feb16795e`, with 9 changed files and no inline review threads. Exact-head workflows `35261887577` (Klim poemx1 invocation bindings), `35261887674` (historical template dependency), `35261887574` (Klim source revisions), `35261887768` (Pages), `35261887595` (frozen diagnostic), and `35261887578` (pinned pylem provider) all completed successfully. The dedicated binding job reports exact head `6141931a13043fa3225971f10ff6c3024a40042a`, checked out that head, ran the standard-library suite, re-fetched pinned oldid `2366546`, regenerated the source-free binding evidence and byte-compared it to the committed artifact. Independent review re-checked current official MediaWiki template documentation and found the bounded anonymous/named whitespace, numbering and duplicate-assignment semantics consistent with the implementation boundary; the intentionally narrower literal-name grammar is non-blocking because all six frozen calls have zero named arguments and unsupported names fail closed. No FantLab source identity was promoted and M2 remains 0/5.
...
ISSUE:          #109
STATUS:         POEMX1_INVOCATION_BINDINGS_MERGED_CONTINUATION_OPEN
PR:             #115 (merged)
MERGED_COMMIT:  16cd22d4f3035bb2b3ec83e810cbe41048015486
NEXT_ACTION:    Continue Issue #109 with only the evidenced historical MediaWiki expansion under the now-frozen frame
                shape: parameter 1 is defined empty, parameter 2 is defined non-empty, and 3/fixed/poem/small/width are
                omitted. Reproduce the required #expr/#if/#ifeq/#iferror and #tag:poem behavior while preserving recursive
                inserted-value expansion and the inferred historical-template anchor as explicit boundaries. Do not claim
                resolved Part 2 bytes until the complete target-only expansion replays deterministically. Only then proceed
                to fail-closed four-part extraction/composition.
```
...
| --- | --- | --- | --- | --- |
| P0 | SCRIP-CORPUS-020 continuation | corpus / provenance | Reproduce only evidenced `#expr` / `#if` / `#ifeq` / `#iferror` and `#tag:poem` behavior under frozen call frame shape | Historical template revision remains an inferred as-of anchor; recursive inserted-value expansion and render equivalence must not be guessed |
...
- SCRIP-CORPUS-020 first pinned the Klim Part 2 dependency, PR #111 corrected the public explanation to target-only `#lst`, PR #112 exposed the exact source-free `poemx1` as-of anchor, PR #113 exposed the post-inclusion effective dependency boundary, PR #114 exposed the literal parameter surface, and merged PR #115 now exposes source-free concrete invocation/frame binding: all six calls define `1` empty and `2` non-empty while omitting the other frozen surface names. Parser/extension expansion and historical render equivalence remain explicit unresolved boundaries.
...
9. Klim Samgin's parent/dependency revisions, target-only `#lst` semantics, inferred historical `poemx1` revision, inclusion controls, post-selection digest, literal parameter-reference/default surface, and the six concrete invocation bindings are frozen. The remaining historical expansion layer is recursive expansion of inserted argument wikitext plus the evidenced `#expr`, `#if`, `#ifeq`, `#iferror`, and `#tag:poem` behavior. Resolved Part 2 bytes, candidate-specific extraction/composition and composite raw/normalized body digests remain unfrozen; no historical render or FantLab-input identity may be inferred yet.
