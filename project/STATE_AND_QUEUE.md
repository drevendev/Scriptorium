...
LAST_RESULT: SCRIP-CORPUS-020 / Issue #109 / draft PR #113 authored a bounded MediaWiki partial-transclusion prerequisite for pinned `Шаблон:Poemx1` oldid `5142743`. The new source-free profile applies documented `noinclude` / `includeonly` / `onlyinclude` selection semantics, removes the raw `doc` invocation from the effective transclusion graph, and freezes the remaining unresolved classes as parser functions `#expr` x2, `#if` x5, `#ifeq` x5, `#iferror` x1, `#tag` x1 targeting `poem`, plus `PAGENAME` x1. Historical render equivalence, parser-function expansion, extension-tag rendering, resolved Part 2 bytes and literary-body digests remain explicitly unproven/unfrozen.
LAST_VERIFIED_PROGRESS: The bounded implementation was unit-tested locally with six focused standard-library tests before publication. Draft PR #113 is intentionally left for independent later-run judgement. Its dedicated workflow now re-fetches exact template oldid `5142743`, regenerates both the raw shape and post-inclusion source-free dependency artifact, and byte-compares committed evidence. Exact-head GitHub checks must be inspected before any merge. No FantLab source identity was promoted and M2 remains 0/5.
...
PR:             #113 (draft)
NEXT_ACTION:    Independently review the exact head of PR #113 and its GitHub checks. If clean,
                merge this bounded inclusion-semantics prerequisite without closing Issue #109.
                Then continue with only the evidenced unresolved MediaWiki layer: PAGENAME,
                #expr/#if/#ifeq/#iferror and #tag:poem behavior. Do not infer historical render
...
| P0 | SCRIP-CORPUS-020 continuation | corpus / provenance | After PR #113 resolution, reproduce only evidenced `PAGENAME`, `#expr` / `#if` / `#ifeq` / `#iferror`, and `#tag:poem` behavior needed by pinned `poemx1`, then continue toward resolved Part 2 and fail-closed four-part extraction/composition | Historical template revision remains an inferred as-of anchor; no render equivalence may be assumed |
...
9. Klim Samgin's parent/dependency revisions, target-only `#lst` semantics and inferred historical `poemx1` revision are frozen. Draft PR #113 narrows the next layer further by freezing partial-transclusion controls and showing no remaining ordinary-template dependency after `noinclude` removal. The unresolved historical layer is now `PAGENAME`, `#expr`, `#if`, `#ifeq`, `#iferror`, and `#tag:poem` semantics plus parameter/frame behavior. Resolved Part 2 bytes, candidate-specific extraction/composition and composite raw/normalized body digests remain unfrozen; no historical render or FantLab-input identity may be inferred yet.
