...
LAST_COMMITTED_RUN_AT: 2026-09-21T07:59:00Z
LAST_RESULT: SCRIP-CORPUS-047 / Issue #181 recovery repaired both merge blockers from independent review `5263767395` on Draft PR #182. The proofread-help evidence is now permanently pinned to `Справка:Вычитка@oldid=5731079` / `2026-07-19`, and `build_evidence()` recomputes the full canonical semantic-backlog self-digest before selecting `ё`, with a regression proving that non-target mutation plus stale digest fails closed. Repaired exact head `d9d4715ab4ac616d34181b3532814e7df1e1b728` remains against unchanged `master@ead874a699c9f1a98c100974835f1ffd6601a21e`. Dedicated run `35575493162` / job `106256295413` passed on that exact SHA, deterministic rebuild reproduced evidence SHA-256 `9123d9f2901b4995deac5c4e09ca09af386befc8129f62e844446d44b864bfd2`, and artifact `10627564488` was independently re-downloaded as a 1,561-byte source-free ZIP with SHA-256 `4da8bbdd75fd025a50117110ef56c5a39ef12b77d9c3fc1573a839fb123eba8e`. Pages run `35575493225` / verify-build job `106256299713` also succeeded on the exact head. At handoff 12/15 PR-triggered workflows had completed successfully; pinned-pylem, frozen-diagnostic and one Klim-Samgin workflow were still queued/in progress with no failure observed. PR #182 remains Draft because this wake authored the repair.
LAST_VERIFIED_PROGRESS: Darwin/Rachinsky has a fail-closed, source-free documentation-evidence boundary for `{{ё}}` that now freezes both supporting documentation revisions and cryptographically verifies the full reviewed semantic backlog before derivation. The documented semantic class remains conditional `е`/`ё`, while historical template/dependency equivalence remains unproven because the retained 2018/2022 Page revisions predate the 2024 template documentation. All 43 backlog shapes therefore remain unresolved; the render profile/backlog are unchanged and `ё` remains the priority-1 historical-template research target with 2,227 invocations. Renderer semantics/implementation/equivalence, inter-page composition, literary-body counts/digests, >=300k admission, FantLab source match, diagnostic readiness and M2 parity remain closed; M2 remains 0/5.
...
PR:             #182 (Draft; repaired exact head d9d4715ab4ac616d34181b3532814e7df1e1b728)
NEXT_ACTION:    Let the remaining exact-head workflows settle, then independently review repaired PR #182.
                Verify the pinned proofread-help revision, full backlog self-digest validation, canonical evidence rebuild,
                and that current documentation is not being used as historical-equivalence proof. If clean, Ready/merge
                in a separate judgement run; otherwise repair. A later bounded unit may freeze historical Шаблон:ЕЁ /
                Шаблон:ё revision/dependency identities at retained Page-save anchors before renderer-profile promotion.
```
...
- **On the Origin of Species — Rachinsky translation** — SCRIP-CORPUS-047 is repaired and remains in Draft PR #182 on top of the independently reviewed SCRIP-CORPUS-046 surface. Master still retains 418 exact Page revision identities, reviewed 388-literary/30-apparatus composition, exact-388 source-free markup inventory, backing Commons DjVu identity, fail-closed render profile SHA-256 `2ebcfea51282505c28824f54f6a1795bd7e87e2f958ac59f5c843e68c2066f3d`, and unresolved-semantic backlog SHA-256 `4ec4f84566f5f3f8dccd1945906d1a782f4532547a13cadc8570cb5b682bdcb2`. PR #182 adds source-free documentation evidence SHA-256 `9123d9f2901b4995deac5c4e09ca09af386befc8129f62e844446d44b864bfd2`: official template documentation and proofread help are both revision-pinned, the full source backlog self-digest is verified before derivation, current documentation establishes that `{{ё}}` is conditional `е`/`ё`, and a retained 2018 Page witness proves that the 2024 template documentation cannot establish historical render equivalence. The profile/backlog therefore remain unchanged with all 43 shapes unresolved and `ё` still priority 1 at 2,227 invocations. Renderer implementation/equivalence, body count/digests, >=300k admission and FantLab source match remain closed; M2 weight remains zero.
...
- The Darwin/Rachinsky semantic-backlog companion now distinguishes documented current `{{ё}}` semantics (conditional `е`/`ё`) from historical render equivalence, publishes both revision-pinned documentation surfaces and the repaired source-free evidence digest, and explains why the 2018 Page witness keeps the profile/backlog unresolved.
...
14. Darwin/Rachinsky 1864 has independently reviewed source-free 388/30 composition, exact-388 markup inventory, backing scan identity, fail-closed decision profile and deterministic unresolved-semantic backlog. SCRIP-CORPUS-047 now pins both documentation surfaces and fail-closes on any stale semantic-backlog digest, while still narrowing `{{ё}}` only to documented conditional `е`/`ё` semantics. The 2024 template-documentation revision postdates retained 2018/2022 Page saves, so historical `Шаблон:ЕЁ` / `Шаблон:ё` revision/dependency identity remains required before profile promotion; all 43 backlog rows stay unresolved. Body count/digests and >=300k admission remain unresolved, and FantLab's current Russian translation surface is a different translation.
