...
LAST_COMMITTED_RUN_AT: 2026-09-18T01:55:00Z
LAST_RESULT: SCRIP-CORPUS-020 / Issue #109 / Draft PR #119 now source-freezes the exact top-level `#tag:poem` call reached by pinned `Шаблон:Poemx1` oldid `5142743` and binds all six frozen parameter-2 identities to it. Exact-source discovery found one content argument and **zero tag attributes**, eliminating the previously open candidate-specific attribute-expression branch. MediaWiki core recursive parsing, Poem output transformation/deployment equivalence, resolved Part 2 bytes, four-part literary-body identity and FantLab source equivalence remain unresolved.
LAST_VERIFIED_PROGRESS: Dedicated discovery workflow `35297169381` checked out authored head `a7e3fde2867cc30d09da9b17db2c96bc43300752`, passed 230 standard-library tests, re-fetched exact template oldid `5142743`, verified the frozen source digest, reapplied partial-transclusion controls and generated the source-free tag surface. The reached call is 22 ASCII bytes with SHA-256 `802765da0c42ff7a629167637cb97926023e0a60f11e58a2677849166c958233`; its sole 8-byte content expression is one empty-default reference to parameter `2`, SHA-256 `64e1af6f2e37ee4bcf0ab4f139c75b671321279929eade54fae4cf49be0346f9`. The workflow was tightened to require byte-identical replay of the committed artifact. M2 remains 0/5.
...
ISSUE:          #109
STATUS:         POEM_TAG_SURFACE_AUTHORED_REVIEW_PENDING
PR:             #119 (draft; independent later-run review required)
MERGED_COMMIT:  none for this slice
NEXT_ACTION:    Independently review exact final head of PR #119 and required checks. If sound, merge without
                closing #109. Then reproduce only the MediaWiki-core recursive-parse behavior material to the
                six plain parameter-2 values plus the bounded Poem transformation needed for source/body identity;
                do not add generic parser behavior that the frozen source cannot reach.
```
...
| --- | --- | --- | --- | --- |
| P0 | SCRIP-CORPUS-020 review / continuation | recovery then corpus / provenance | Independently review Draft PR #119; after merge, reproduce only the MediaWiki-core recursive parse and bounded Poem transformation material to the six exact plain values | Reached `#tag:poem` has one content argument and zero attributes; actual Wikisource Poem deployment and historical render equivalence remain unproven |
...
- SCRIP-CORPUS-020 first pinned the Klim Part 2 dependency, PR #111 corrected the public explanation to target-only `#lst`, PR #112 exposed the exact source-free `poemx1` as-of anchor, PR #113 exposed the post-inclusion effective dependency boundary, PR #114 exposed the literal parameter surface, merged PR #115 exposed concrete invocation/frame binding, merged PR #116 exposed source-free control reachability, merged PR #117 added a source-free parameter-2 surface showing that all six exact values require no additional nested template/parser dependency resolution, merged PR #118 added a source-free inferred Poem implementation anchor, and Draft PR #119 now exposes that the live `#tag:poem` call has exactly one content argument and zero attributes. Historical render equivalence and Part 2/body identity remain explicit unresolved boundaries.
...
9. Klim Samgin's parent/dependency revisions, target-only `#lst` semantics, inferred historical `poemx1` revision, inclusion controls, post-selection digest, literal parameter-reference/default surface, six concrete invocation bindings, frozen-frame `#if`/`#ifeq` control reachability, exact parameter-2 expansion surface, inferred upstream Poem source anchor, and reached `#tag:poem` source surface are frozen. The reached tag has one content argument and zero attributes, so no candidate-specific tag-attribute parser branch remains. The Poem anchor is still not evidence of the extension commit actually deployed by Russian Wikisource. MediaWiki core recursive parsing, Poem output transformation, resolved Part 2 bytes, candidate-specific extraction/composition and composite raw/normalized body digests remain unfrozen; no historical render or FantLab-input identity may be inferred yet.
