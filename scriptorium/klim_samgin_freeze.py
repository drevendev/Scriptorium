"""Source-free structural probing and freezing helpers for Klim Samgin.

The probe reports markup inventory only. Source prose remains transient in process memory.
A later commit in the same bounded unit may promote the observed shape into a strict
literary-body extraction contract once the pinned revisions have been inspected.
"""
...
def probe_revision_shape(revision_manifest: Mapping[str, object]) -> dict[str, object]:
    ...
    return {
        "probe_version": "scriptorium-klim-samgin-shape-probe-v1",
