"""Reduce hosted Twelve Chairs markup audit to durable source-free evidence."""
...
from .twelve_chairs_page_freeze import CANDIDATE_ID, EXPECTED_PAGE_COUNT, FAMILY_ID
from .twelve_chairs_render_surface import PROFILE_STATUS, validate_audit_manifest

FREEZE_VERSION = "scriptorium-twelve-chairs-render-surface-freeze-v1"
...
def validate_freeze_manifest(manifest: Mapping[str, object]) -> None:
    if manifest.get("schema_version") != FREEZE_VERSION:
        raise ValueError("Twelve Chairs rendering-surface freeze schema drift")
    if manifest.get("candidate_id") != CANDIDATE_ID or manifest.get("family_id") != FAMILY_ID:
        raise ValueError("Twelve Chairs rendering-surface freeze identity drift")
    if manifest.get("status") != PROFILE_STATUS or manifest.get("dependency_count") != EXPECTED_PAGE_COUNT:
        raise ValueError("Twelve Chairs rendering-surface freeze status/dependency drift")
    if manifest.get("page_surface_receipts_count") != EXPECTED_PAGE_COUNT:
        raise ValueError("Twelve Chairs rendering-surface freeze receipt count drift")
    if manifest.get("identity_replay_match") is not True or manifest.get("source_text_included") is not False:
        raise ValueError("Twelve Chairs rendering-surface freeze source boundary drift")
    for key in (
    ...
    for key in ("template_shapes", "tag_shapes", "construct_totals", "page_surface_receipts_sha256"):
    ...
    if digest != _sha256_json(unsigned):
        raise ValueError("Twelve Chairs rendering-surface freeze digest drift")
