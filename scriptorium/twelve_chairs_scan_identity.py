"""Freeze the Twelve Chairs 1928 Commons PDF byte identity without persisting scan bytes.
...
RECEIPT_VERSION = "scriptorium-twelve-chairs-scan-identity-v1"
SCAN_URL = (
    "https://upload.wikimedia.org/wikipedia/commons/0/00/"
    "%D0%98%D0%BB%D1%8C%D1%84_%D0%98._%D0%9F%D0%B5%D1%82%D1%80%D0%BE%D0%B2_%D0%95._"
    "%D0%94%D0%B2%D0%B5%D0%BD%D0%B0%D0%B4%D1%86%D0%B0%D1%82%D1%8C_%D1%81%D1%82%D1%83%D0%BB%D1%8C%D0%B5%D0%B2_"
    "%28%D0%97%D0%B8%D0%A4%2C_1928%29.pdf"
)
FILE_PAGE_URL = (
...
EDITION_IDENTITY = "1928_first_standalone_zemlya_i_fabrika_facsimile"
EXPECTED_BYTE_COUNT = 77_978_350
EXPECTED_SHA1 = "4ab6aa42c3517169e99c1177b6fb6412cfe9187d"
...
def validate_identity(identity: Mapping[str, object]) -> None:
    if set(identity) != {"byte_count", "sha1", "sha256"}:
        raise ValueError("Twelve Chairs scan identity shape drift")
    byte_count = identity.get("byte_count")
    if not isinstance(byte_count, int) or isinstance(byte_count, bool) or byte_count <= 0:
        raise ValueError("Twelve Chairs scan byte count is invalid")
    for name, length in (("sha1", 40), ("sha256", 64)):
        digest = identity.get(name)
        if not isinstance(digest, str) or len(digest) != length:
            raise ValueError(f"Twelve Chairs scan {name} is missing or malformed")
        try:
            int(digest, 16)
        except ValueError as exc:
            raise ValueError(f"Twelve Chairs scan {name} is not hexadecimal") from exc


def validate_provider_identity(identity: Mapping[str, object]) -> None:
    validate_identity(identity)
    if identity["byte_count"] != EXPECTED_BYTE_COUNT:
        raise ValueError("Twelve Chairs Commons byte count drift")
    if identity["sha1"] != EXPECTED_SHA1:
        raise ValueError("Twelve Chairs Commons SHA-1 drift")
...
def build_receipt(chunks: Iterable[bytes] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "page_dependency_set_changed": False,
        "gap_pages_classified": False,
...
def validate_receipt(receipt: Mapping[str, object]) -> None:
    required = {
        ...
        "page_dependency_set_changed",
        "gap_pages_classified",
    ...
    if set(receipt) != required:
        raise ValueError("Twelve Chairs scan receipt shape drift or unexpected payload key")
    if receipt.get("receipt_version") != RECEIPT_VERSION or receipt.get("candidate_id") != CANDIDATE_ID:
        raise ValueError("Twelve Chairs scan receipt identity drift")
    if receipt.get("provider") != "Wikimedia Commons" or receipt.get("edition_identity") != EDITION_IDENTITY:
        raise ValueError("Twelve Chairs scan provider/edition drift")
    if receipt.get("file_page_url") != FILE_PAGE_URL or receipt.get("exact_original_url") != SCAN_URL:
        raise ValueError("Twelve Chairs scan locator drift")
    provider_identity = receipt.get("provider_reported_identity")
    if provider_identity != {"byte_count": EXPECTED_BYTE_COUNT, "sha1": EXPECTED_SHA1}:
        raise ValueError("Twelve Chairs provider-reported identity drift")
    identity = receipt.get("identity")
    if not isinstance(identity, Mapping):
        raise ValueError("Twelve Chairs scan identity is missing")
    validate_provider_identity(identity)
    if receipt.get("binary_bytes_retrieved_transiently") is not True:
        raise ValueError("Twelve Chairs scan receipt must record transient binary retrieval")
    expected_false = (
        "binary_bytes_committed",
        "ocr_included",
        "literary_source_text_included",
    ...
    if any(receipt.get(key) is not False for key in expected_false):
        raise ValueError("Twelve Chairs scan receipt advanced a forbidden gate or persisted source")
    if receipt.get("fantlab_source_edition_match") != "unknown":
        raise ValueError("Twelve Chairs scan receipt advanced FantLab source identity")
...
def verify_receipt(receipt: Mapping[str, object], chunks: Iterable[bytes] | None = None) -> dict[str, object]:
    ...
    if observed != dict(receipt):
        raise ValueError("Twelve Chairs scan byte identity no longer matches frozen receipt")
