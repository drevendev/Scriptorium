...
_RIGHT_RE = re.compile(r"\{\{right\|([^{}]*)\}\}", re.IGNORECASE)
_RAZR_RE = re.compile(r"\{\{razr\|([^{}]*)\}\}", re.IGNORECASE)
...
_HTML_TAG_RE = re.compile(r"</?(?:span|small|big|i|b|em|strong|sup|sub|br|center)\b[^>]*>", re.IGNORECASE)
...
def extract_transcription_body(wikitext: str) -> str:
    ...
    while previous != body:
        ...
        body = _RIGHT_RE.sub(lambda match: match.group(1), body)
        body = _RAZR_RE.sub(lambda match: match.group(1), body)
