...
class KlimSamginRevisionManifestTests(unittest.TestCase):
    def test_four_part_manifests_are_source_free_and_exactly_ordered(self) -> None:
        ...
        for part, expected_revision, expected_title in zip(
            range(1, 5), EXPECTED_REVISIONS, EXPECTED_TITLES, strict=True
        ):
            manifest = _load(
                TRACE_DIR / f"gorky-klim-samgin-ru.part{part}.revision.json"
            )
        ...
        self.assertEqual(
            ...
            sum(int(row["wikitext_character_count"]) for row in rows),
        )
        self.assertEqual(
    ...
    def test_summary_stays_fail_closed_for_body_and_fantlab_identity(self) -> None:
        ...
        self.assertEqual(
            [row["revision_id"] for row in summary["part_manifests"]],
            list(EXPECTED_REVISIONS),
        )
