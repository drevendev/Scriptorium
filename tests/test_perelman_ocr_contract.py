...
from scriptorium.perelman_ocr_contract import (
    CANDIDATE_ID,
    CONTRACT_VERSION,
    SCAN_IDENTITY,
    canonical_sha256,
...
CONTRACT_PATH = Path(
    ...
    "perelman-entertaining-physics-book1-1913-ru.ocr-contract.json"
)
...
class PerelmanOcrContractTests(unittest.TestCase):
    ...
    def test_page_selection_cannot_be_silently_bound_in_v1(self) -> None:
        contract = production_contract()
        contract["page_selection"] = {
            "status": "bound",
            "included_pdf_pages": [5, 6],
            "excluded_pdf_pages": [1, 2, 3, 4],
            "evidence": {"note": "invented"},
        }
        with self.assertRaisesRegex(ValueError, "page selection"):
            validate_contract(contract)
    ...
    def test_ocr_or_rasterizer_identity_cannot_be_invented_in_v1(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "OCR toolchain"):
            validate_contract(contract)
